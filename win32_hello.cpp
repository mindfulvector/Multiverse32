// win32_hello.cpp - Main source file for Win32 Hello World
// This file contains the implementation from win32_compat.h
...
void* CreatePlatformWindow(const char* title, int x, int y, int width, int height) {
    cout << "=== CreatePlatformWindow Debug ===" << endl;
    ...
    @autoreleasepool {
        ...
        if (window) {
            cout << "UIWindow created successfully: " << window << endl;
            [window setBackgroundColor:[UIColor whiteColor]];
            [window setHidden:NO];
            [window setRootViewController:[[UIViewController alloc] init]];
            [window setTitle:[NSString stringWithUTF8String:title]];
        } else {
            cout << "ERROR: Failed to create UIWindow!" << endl;
        }
        return (__bridge_retained void*)window;
    }
}
